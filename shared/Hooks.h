...
#define CREATE_HOOK(startDisabled, target, ret, callconv, name, ...)	\
	...
	{													\
		...
		static inline name##_t original = NULL;				\
															\
		...
		sHooked_##name() { if (MH_CreateHook((LPVOID)(target), hk##name, (LPVOID*)&original) != MH_OK) _wassert(L#name " hook creation failed!", L"", 0); if constexpr (!startDisabled) Enable(); }  \
